...
class SessionManager:
    ...
    def hashcat_action(self, session, action):
        # First get the session.
        # session = self.get(session_id=session_id)[0]
        ...
        if action == 'start':
            ...
            command = self.hashcat.build_command_line(
                ...
                self.session_filesystem.get_hashfile_path(session.user_id, session.id),
                session.hashcat.wordlist_path,
                session.hashcat.rule_path,
                self.session_filesystem.get_crackedfile_path(session.user_id, session.id),
                self.session_filesystem.get_potfile_path(session.user_id, session.id),
            ...
            screen.set_logfile(self.session_filesystem.get_screenfile_path(session.user_id, session.id))
    ...
    def terminate_past_sessions(self):
        ...
        for past_session in past_sessions:
            ...
            if status == 1 or status == 4:
                ...
                self.hashcat_action(session, 'stop')
